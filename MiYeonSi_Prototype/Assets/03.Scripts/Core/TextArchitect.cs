...
public class TextArchitect
{
    ...
    private class ENCAPSULATED_TEXT
    {
        ...
        //current text is the currently cuilt target text without tages. target text is the cuild target.
        ...
        //contains elements that the encapsulater will attempt to advance to when searching for sub encapsulators.
        ...
        public ENCAPSULATED_TEXT(string tag, string[] allSpeechAndTagsArray, int arrayProgress)
        {
            ...
            if(allSpeechAndTagsArray.Length - 1 > arrayProgress)
            {
                ...
                //if the next writable part is not a tag, make it the target text o this encapsulator.
                if (!isATag)
                    targetText = nextPart;
                //if the next writable part is a tag, add a sub encapsulator to take care of it. else구문은 하지 말라고 써있음.
                else
                {
        ...
        /// <summary>
        /// Take the next step in the construction process. returns true if a step was taken. returns false if a step must be made from a lower level encapsulate.
        /// </summary>
        public bool Step()
        {
            ...
            else
            {
                ...
                {
                    //if there si still more dialogue to build.
                    if(allSpeechAndTagsArray.Length > arrayProgress +1 )
                    {
                        ...
                        //if the next part is not a tag, then this is an extension to be added to the encapsuulator's target.
                        else
                        {
                            targetText += nextPart;
                            UpdateArrayProgress();
